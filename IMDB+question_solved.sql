...
-- Number of movies released accordingly each year
SELECT year AS Year, COUNT(id) AS number_of_movies
FROM movie
GROUP BY year
ORDER BY year;

-- Number of movies released accordingly each month
...
(
	SELECT movie_id, 
			COUNT(genre) AS movies_cnt
	FROM genre
	GROUP BY movie_id
	HAVING movies_cnt=1
)

SELECT COUNT(movie_id) AS movies_cnt
FROM one_genre;
...
SELECT d.name_id, name, d.movie_id,
	   m.date_published, 
       LEAD(date_published, 1) OVER(PARTITION BY d.name_id ORDER BY date_published, d.name_id) AS next_movie_date
